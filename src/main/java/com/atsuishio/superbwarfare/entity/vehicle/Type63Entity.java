...
public class Type63Entity extends ContainerMobileVehicleEntity implements GeoEntity, OBBEntity {
    ...
    public @NotNull InteractionResult interact(Player player, @NotNull InteractionHand hand) {
        ...
        if (stack.isEmpty()) {
            if (player.isShiftKeyDown()) {
                if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == hoe1) {
                    if (player.level() instanceof ServerLevel serverLevel) {
                        ...
                        if (cooldown == 0) {
                            ...
                            serverLevel.playSound(null, vec3.x, vec3.y, vec3.z, ModSounds.WHEEL_STEP, SoundSource.PLAYERS, 0.5f, random.nextFloat() * 0.05f + 0.975f);
                        }
                    }
                    player.swing(InteractionHand.MAIN_HAND);
                }
                if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == hoe2) {
                    if (player.level() instanceof ServerLevel serverLevel) {
                        entityData.set(BODY_YAW, entityData.get(BODY_YAW) - 0.2f * (float) interactionTick);
                        interactionTick++;
                        if (cooldown == 0) {
            ...
            if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == yawController) {
            ...
            if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == pitchController) {
            ...
            if (player.isShiftKeyDown()) {
                for (int i = 0; i < this.barrel.length; i++) {
                    if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == this.barrel[i] && !items.get(i).isEmpty() && level() instanceof ServerLevel serverLevel && cooldown == 0) {
        ...
        if (stack.is(ModTags.Items.CROWBAR) || stack.is(Items.FLINT_AND_STEEL)) {
            ...
            if (lookingAtBarrel(player)) {
                // 精准发射
                for (int i = 0; i < this.barrel.length; i++) {
                    if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == this.barrel[i] && items.get(i).getItem() instanceof MediumRocketItem && cooldown == 0) {
                        shoot(player, i);
                        items.set(i, ItemStack.EMPTY);
                        setChanged();
                        player.swing(InteractionHand.MAIN_HAND);
    ...
    public boolean lookingAtBarrel(Player player) {
        for (int i = 0; i < 12; i++) {
            if (OBB.getLookingObb(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH)) == barrel[i]) {
                return true;
            }
        }
        return false;
    ...
    public void shoot(Player player, int i) {
        ...
        level().playSound(null, shootPos.x, shootPos.y, shootPos.z, ModSounds.MEDIUM_ROCKET_FIRE, SoundSource.PLAYERS, 4f, random.nextFloat() * 0.1f + 0.95f);
