...
public class AmmoBarOverlay {
    ...
    public static void render(GuiGraphics guiGraphics, float partialTick) {
        ...
        if (stack.getItem() instanceof GunItem gunItem && !(player.getVehicle() instanceof ArmedVehicleEntity vehicle && vehicle.banHand(player))) {
            int x = guiGraphics.guiWidth() + DisplayConfig.WEAPON_HUD_X_OFFSET.get();
            int y = guiGraphics.guiHeight() + DisplayConfig.WEAPON_HUD_Y_OFFSET.get();
            ...
            if (stack.getItem() != ModItems.MINIGUN) {
                guiGraphics.drawString(
                        font,
                        "[" + ModKeyMappings.FIRE_MODE.key.getDisplayName().toString() + "]",
                        (int) (x - 111.5),
                        y - 20,
                        0xFFFFFF,
                        false
            ...
            if (data.virtualAmmo.get() > 0 && !data.meleeOnly()) {
                guiGraphics.drawString(
                        ...
                        (int) (x - 62 + font.width(getGunAmmoString(data, player)) * 1.5),
                        y - 46,
                        0x55FFFF,
                        true
                );
