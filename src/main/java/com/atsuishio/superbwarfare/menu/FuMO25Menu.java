...
public class FuMO25Menu extends EnergyMenu {
    ...
    public static void onContainerOpened(ServerPlayer player, AbstractContainerMenu container) {
        if (container instanceof FuMO25Menu fuMO25Menu) {
            fuMO25Menu.getSelfPos().ifPresent(pos ->
                    new RadarMenuOpenMessage(pos).send(player));
        }
    }
    ...
    public static void onContainerClosed(ServerPlayer player, AbstractContainerMenu container) {
        if (container instanceof FuMO25Menu) {
            ServerPlayNetworking.send(player, RadarMenuCloseMessage.ID, RadarMenuCloseMessage.INSTANCE);
        }
    }
}
