    ...
    public static void render(GuiGraphics guiGraphics, float partialTick) {
        ...
        if (player.getVehicle() instanceof HelicopterEntity iHelicopterEntity && player.getVehicle() instanceof MobileVehicleEntity mobileVehicle && iHelicopterEntity.isDriver(player) && player.getVehicle() instanceof WeaponVehicleEntity weaponVehicle) {
            ...
            if (Minecraft.getInstance().options.getCameraType() == CameraType.FIRST_PERSON) {
                ...
                RenderHelper.blit(poseStack, Mod.loc("textures/screens/compass.png"), (float) guiGraphics.guiWidth() / 2 - 128, (float) 6, 128 + ((float) 64 / 45 * mobileVehicle.getYRot()), 0, 256, 16, 512, 16, color);

                poseStack.pushPose();
                poseStack.rotateAround(Axis.ZP.rotationDegrees(-iHelicopterEntity.getRotZ(partialTick)), guiGraphics.guiWidth() / 2f, guiGraphics.guiHeight() / 2f, 0);
                float pitch = iHelicopterEntity.getRotX(partialTick);

                RenderHelper.blit(poseStack, Mod.loc("textures/screens/helicopter/heli_line.png"), (float) guiGraphics.guiWidth() / 2 - 128, (float) guiGraphics.guiHeight() / 2 - 512 - 5.475f * pitch, 0, 0, 256, 1024, 256, 1024, color);
                poseStack.popPose();
                poseStack.pushPose();
                poseStack.rotateAround(Axis.ZP.rotationDegrees(iHelicopterEntity.getRotZ(partialTick)), guiGraphics.guiWidth() / 2f, guiGraphics.guiHeight() / 2f - 56, 0);
                RenderHelper.blit(poseStack, Mod.loc("textures/screens/helicopter/roll_ind.png"), (float) guiGraphics.guiWidth() / 2 - 8, (float) guiGraphics.guiHeight() / 2 - 88, 0, 0, 16, 16, 16, 16, color);
                poseStack.popPose();

                RenderHelper.blit(poseStack, Mod.loc("textures/screens/helicopter/heli_power_ruler.png"), (float) guiGraphics.guiWidth() / 2 + 100, (float) guiGraphics.guiHeight() / 2 - 64, 0, 0, 64, 128, 64, 128, color);
                ...
                RenderHelper.blit(poseStack, Mod.loc("textures/screens/helicopter/heli_power.png"), (float) guiGraphics.guiWidth() / 2 + 130f, ((float) guiGraphics.guiHeight() / 2 - 64 + 124 - power * 980), 0, 0, 4, power * 980, 4, power * 980, color);
                lerpVy = (float) Mth.lerp(0.021f * partialTick, lerpVy, mobileVehicle.getDeltaMovement().y());
                RenderHelper.blit(poseStack, Mod.loc("textures/screens/helicopter/heli_vy_move.png"), (float) guiGraphics.guiWidth() / 2 + 138, ((float) guiGraphics.guiHeight() / 2 - 3 - Math.max(lerpVy * 20, -24) * 2.5f), 0, 0, 8, 8, 8, 8, color);
                guiGraphics.drawString(Minecraft.getInstance().font, Component.literal(FormatTool.format0D(lerpVy * 20, "m/s")),
                        guiGraphics.guiWidth() / 2 + 146, (int) (guiGraphics.guiHeight() / 2F - 3 - Math.max(lerpVy * 20, -24) * 2.5), (lerpVy * 20 < -24 || ((lerpVy * 20 < -10 || (lerpVy * 20 < -1 && length(mobileVehicle.getDeltaMovement().x, mobileVehicle.getDeltaMovement().y, mobileVehicle.getDeltaMovement().z) * 72 > 100)) && height < 36) || (length(mobileVehicle.getDeltaMovement().x, mobileVehicle.getDeltaMovement().y, mobileVehicle.getDeltaMovement().z) * 72 > 40 && blockInWay < 72) ? -65536 : color), false);
                guiGraphics.drawString(Minecraft.getInstance().font, Component.literal(FormatTool.format0D(mobileVehicle.getY())),
                        guiGraphics.guiWidth() / 2 + 104, guiGraphics.guiHeight() / 2, color, false);
                RenderHelper.blit(poseStack, Mod.loc("textures/screens/helicopter/speed_frame.png"), (float) guiGraphics.guiWidth() / 2 - 144, (float) guiGraphics.guiHeight() / 2 - 6, 0, 0, 50, 18, 50, 18, color);
                guiGraphics.drawString(Minecraft.getInstance().font, Component.literal(FormatTool.format0D(length(mobileVehicle.getDeltaMovement().x, mobileVehicle.getDeltaMovement().y, mobileVehicle.getDeltaMovement().z) * 72, "km/h")),
                        guiGraphics.guiWidth() / 2 - 140, guiGraphics.guiHeight() / 2, color, false);
                ...
                if (mobileVehicle instanceof Ah6Entity ah6Entity) {
                    if (weaponVehicle.getWeaponIndex(0) == 0) {
                        ...
                        guiGraphics.drawString(Minecraft.getInstance().font, Component.literal("20MM CANNON " + (InventoryTool.hasCreativeAmmoBox(player) ? "∞" : iHelicopterEntity.getAmmoCount(player))), guiGraphics.guiWidth() / 2 - 160, guiGraphics.guiHeight() / 2 - 60, MathTool.getGradientColor(color, 0xFF0000, heat, 2), false);
                    } else {
                        guiGraphics.drawString(Minecraft.getInstance().font, Component.literal("70MM ROCKET " + iHelicopterEntity.getAmmoCount(player)), guiGraphics.guiWidth() / 2 - 160, guiGraphics.guiHeight() / 2 - 60, color, false);
                    }
                }

                guiGraphics.drawString(Minecraft.getInstance().font, Component.literal("FLARE " + iHelicopterEntity.getDecoy()), guiGraphics.guiWidth() / 2 - 160, guiGraphics.guiHeight() / 2 - 50, color, false);

                if (lerpVy * 20 < -24) {
                    guiGraphics.drawString(Minecraft.getInstance().font, Component.literal("SINK RATE，PULL UP!"),
                            guiGraphics.guiWidth() / 2 - 53, guiGraphics.guiHeight() / 2 + 24, -65536, false);
                    ...
                    guiGraphics.drawString(Minecraft.getInstance().font, Component.literal("TERRAIN TERRAIN"),
                            guiGraphics.guiWidth() / 2 - 42, guiGraphics.guiHeight() / 2 + 24, -65536, false);
                }
