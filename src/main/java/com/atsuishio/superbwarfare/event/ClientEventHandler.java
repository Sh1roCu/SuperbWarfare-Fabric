...
public class ClientEventHandler {
    ...
    public static void handleClientTick(Minecraft client) {
        ...
        if (stack.is(ModTags.Items.GUN)) {
            ...
            if (holdFire || (zoom && stack.is(ModItems.MINIGUN))) {
                ...
                if (stack.is(ModItems.MINIGUN)) {
                    ...
                    player.playSound(ModSounds.MINIGUN_ROT, 1, 0.7f + rpm);
                }
        ...
        if (!notInGame() && (player.getVehicle() instanceof MobileVehicleEntity mobileVehicle && mobileVehicle.getFirstPassenger() == player)
                || (stack.is(ModItems.MONITOR) && ItemNBTTool.getBoolean(stack, "Using", false) && ItemNBTTool.getBoolean(stack, "Linked", false))
        ) {
        ...
        if (keys != keysCache) {
            new VehicleMovementMessage(keys).send();
            keysCache = keys;
        }
        ...
        if ((stack.is(ModItems.ARTILLERY_INDICATOR) || (stack.is(ModItems.MONITOR) && player.getOffhandItem().is(ModItems.ARTILLERY_INDICATOR))) && holdFire) {
            holdArtilleryIndicator = Mth.clamp(holdArtilleryIndicator + 1, 0, 10);
            if (holdArtilleryIndicator >= 9) {
                ClientPlayNetworking.send(ArtilleryIndicatorFireMessage.ID, ArtilleryIndicatorFireMessage.INSTANCE);
            }
        } else {
        ...
        if (player.getVehicle() instanceof VehicleEntity vehicle && vehicle.allowEjection() && ModKeyMappings.DISMOUNT.isDown()) {
            ...
            if (holdToEjection >= 10) {
                new PlayerStopRidingMessage(true).send();
            }
        } else {
            holdToEjection = 0;
        }
    ...
    public static void staminaSystem() {
        ...
        if (tacticalSprint && (player.onGround() || player.jumping)) {
            new TacticalSprintMessage(true).send();
        } else {
            new TacticalSprintMessage(false).send();
        }
    }
    ...
    public static void doGunMeleeAttack(Player player) {
        ...
        Entity lookingEntity = TraceTool.findMeleeEntity(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH));
        if (lookingEntity != null) {
            new MeleeAttackMessage(lookingEntity.getUUID()).send();
        }
    }

    public static void handleLungeAttack(Player player, ItemStack stack) {
        if (stack.is(ModItems.LUNGE_MINE) && lungeAttack == 0 && lungeDraw == 0 && holdFire) {
            lungeAttack = 18;
            holdFire = false;
            player.playSound(SoundEvents.PLAYER_ATTACK_SWEEP, 1f, 1);
        }
        ...
        if (stack.is(ModItems.LUNGE_MINE) && ((lungeAttack >= 9 && lungeAttack <= 10.5) || lungeSprint > 0)) {
            Entity lookingEntity = TraceTool.findLookingEntity(player, player.getAttributeValue(PortingLibAttributes.ENTITY_REACH) + 1.5);

            BlockHitResult result = player.level().clip(new ClipContext(player.getEyePosition(), player.getEyePosition().add(player.getLookAngle().scale(player.getAttributeValue(PortingLibAttributes.BLOCK_REACH) + 0.5)),
            ...
            if (lookingEntity != null) {
                new LungeMineAttackMessage(0, lookingEntity.getUUID(), result.getLocation()).send();
                lungeSprint = 0;
                lungeAttack = 0;
                lungeDraw = 15;
                ...
                new LungeMineAttackMessage(1, player.getUUID(), result.getLocation()).send();
                lungeSprint = 0;
                lungeAttack = 0;
                lungeDraw = 15;
            }
    ...
    public static void handleWeaponFire(RenderTickEvent event) {
        ...
        } else if (stack.is(ModItems.MINIGUN)) {
            zoomSpread = 1 - (0.25 * zoomTime);
        } else {
            zoomSpread = 1 - (0.9 * zoomTime);
        }

        double spread = stack.is(ModTags.Items.SHOTGUN) || stack.is(ModItems.MINIGUN) ? 1.2 * zoomSpread * (basicDev + 0.2 * (walk + sprint + crouching + prone + jump + ride) + fireSpread) : zoomSpread * (0.7 * basicDev + walk + sprint + crouching + prone + jump + ride + 0.8 * fireSpread);
        ...
        //左轮类
        if (clientTimer.getProgress() == 0 && stack.is(ModItems.TRACHELIUM) && holdFire) {
        ...
        if (stack.getItem() == ModItems.DEVOTION && (GunData.from(stack).reload.normal() || GunData.from(stack).reload.empty())) {
            customRpm = 0;
        }
    }

    public static void beamShoot(Player player, ItemStack stack) {
        if (stack.is(ModItems.BEAM_TEST) && player.getUseItem() == stack) {
            ...
            if (canAttack) {
                new LaserShootMessage(1, lookingEntity.getUUID(), TraceTool.laserHeadshot).send();
            }
        }
    }
    ...
    public static void handleClientShoot() {
        ...
        new ShootMessage(gunSpread, zoom, entity != null ? entity.getUUID() : null).send();
        fireRecoilTime = 10;
    ...
    public static void playGunClientSounds(Player player) {
        ...
        if (stack.getItem() == ModItems.SENTINEL) {
            ...
            if (storage != null) {
                charged[0] = storage.getAmount() > 0;
            }

            if (charged[0]) {
                player.playSound(ModSounds.SENTINEL_CHARGE_FIRE_1P, 2f, (float) ((2 * org.joml.Math.random() - 1) * 0.05f + 1.0f));
                return;
            }
        }
        ...
        if (stack.getItem() == ModItems.SECONDARY_CATACLYSM) {
            var storage = EnergyStorage.ITEM.find(stack, ContainerItemContext.withConstant(stack));
            var hasEnoughEnergy = storage != null && storage.getAmount() >= 3000;

            boolean isChargedFire = zoom && hasEnoughEnergy;

            if (isChargedFire) {
                player.playSound(ModSounds.SECONDARY_CATACLYSM_FIRE_1P_CHARGE, 2f, (float) ((2 * org.joml.Math.random() - 1) * 0.05f + 1.0f));
                return;
            }
        }
        ...
        if (perk == ModPerks.BEAST_BULLET) {
            player.playSound(ModSounds.HENG, 1f, (float) ((2 * org.joml.Math.random() - 1) * 0.1f + pitch));
        }
    ...
    public static void handleVehicleFire(RenderTickEvent event) {
        ClientLevel level = Minecraft.getInstance().level;
        Player player = Minecraft.getInstance().player;
        if (player == null) return;
        if (level == null) return;
        ...
        if (player.getVehicle() instanceof VehicleEntity pVehicle && player.getVehicle() instanceof WeaponVehicleEntity iVehicle && iVehicle.hasWeapon(pVehicle.getSeatIndex(player)) && iVehicle.canShoot(player)) {
            ...
            if ((holdFireVehicle)) {
                ...
                if (clientTimerVehicle.getProgress() >= cooldown) {
                    ...
                    // 低帧率下的开火次数补偿
                    do {
                        new VehicleFireMessage(pVehicle.getSeatIndex(player)).send();
    ...
    public static void handleWeaponBreathSway(RenderTickEvent event) {
        Player player = Minecraft.getInstance().player;
        if (player == null) return;
        ItemStack stack = player.getMainHandItem();
        if (!(stack.getItem() instanceof GunItem gunItem)) return;
    ...
    public static void computeCameraAngles(ViewportEvent.ComputeCameraAngles event) {
        ...
        if (level != null &&
                (stack.is(ModItems.MONITOR) && stack.getOrCreateTag().getBoolean("Using") && stack.getOrCreateTag().getBoolean("Linked"))) {
    ...
    private static void handleBowPullAnimation(LivingEntity entity, ItemStack stack) {
        ...
        if (holdFire && data.hasEnoughAmmoToShoot(entity) && !bowPull && stack.is(ModItems.BOCEK)) {
    ...
    public static boolean setPlayerInvisible(Player player, PlayerRenderer renderer, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        return player.getVehicle() instanceof VehicleEntity vehicle && vehicle.hidePassenger(player);
    }

    public static void handleRenderCrossHair(AtomicBoolean cancelled) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null) {
            return;
     ...
     * 载具banHand时，禁用快捷栏渲染
     */
    public static void handleAvoidRenderingHotbar(AtomicBoolean cancelled) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null) {
            return;
    ...
    public static void aimAtVillager(Player player) {
        ...
        if (zoom) {
            ...
            if (entity instanceof AbstractVillager villager) {
                ...
                for (var e : entities) {
                    if (e == player) {
                        new AimVillagerMessage(villager.getId()).send();
