...
public class CrossHairOverlay {
    ...
    public static void render(GuiGraphics guiGraphics, float partialTick) {
        ...
        if (shouldRenderCrossHair(player) || (Minecraft.getInstance().options.getCameraType() == CameraType.FIRST_PERSON && (stack.is(ModItems.MINIGUN) || stack.is(ModItems.AURELIA_SCEPTRE) || stack.is(ModItems.M_2_HB))) || (Minecraft.getInstance().options.getCameraType() == CameraType.THIRD_PERSON_BACK && (ClientEventHandler.zoomTime > 0 || ClientEventHandler.bowPullPos > 0))) {
            ...
            if (!player.isSprinting() || ClientEventHandler.cantSprint > 0) {
                if (data.get(GunProp.PROJECTILE_AMOUNT) > 1) {
                    ...
                    normalCrossHair(guiGraphics, guiGraphics.guiWidth(), guiGraphics.guiHeight(), spread, moveX, moveY);
                }
            }
        }
        ...
        if (DisplayConfig.KILL_INDICATION.get() && !(player.getVehicle() instanceof Ah6Entity ah6Entity && ah6Entity.getFirstPassenger() == player)) {
            renderKillIndicator(guiGraphics, guiGraphics.guiWidth(), guiGraphics.guiHeight(), moveX, moveY);
        }
    ...
    private static boolean shouldRenderCrossHair(Player player) {
        ...
        return !(player.getMainHandItem().getItem() == ModItems.M_79 || player.getMainHandItem().getItem() == ModItems.BOCEK || player.getMainHandItem().getItem() == ModItems.SECONDARY_CATACLYSM)
