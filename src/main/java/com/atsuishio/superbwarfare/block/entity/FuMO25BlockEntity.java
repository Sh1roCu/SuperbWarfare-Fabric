...
public class FuMO25BlockEntity extends BlockEntity implements MenuProvider, GeoBlockEntity {
    ...
    public final SimpleEnergyStorage energyStorage = new SimpleEnergyStorage(MAX_ENERGY, MAX_ENERGY, MAX_ENERGY_COST);
    ...
    protected final ContainerEnergyData dataAccess = new ContainerEnergyData() {
        ...
        public long get(int pIndex) {
            return switch (pIndex) {
                case 0 -> FuMO25BlockEntity.this.energyStorage.getAmount();
        ...
        public void set(int pIndex, long pValue) {
            switch (pIndex) {
                case 0 -> FuMO25BlockEntity.this.energyStorage.amount = pValue;
    ...
    public static void serverTick(Level pLevel, BlockPos pPos, BlockState pState, FuMO25BlockEntity blockEntity) {
        long energy = blockEntity.energyStorage.getAmount();
        ...
        if (energy < energyCost) {
            if (pState.getValue(FuMO25Block.POWERED)) {
                ...
                pLevel.playSound(null, pPos, ModSounds.RADAR_SEARCH_END, SoundSource.BLOCKS, 1.0F, 1.0F);
            ...
            if (!pState.getValue(FuMO25Block.POWERED)) {
                if (energy >= DEFAULT_MIN_ENERGY) {
                    ...
                    pLevel.playSound(null, pPos, ModSounds.RADAR_SEARCH_START, SoundSource.BLOCKS, 1.0F, 1.0F);
                    blockEntity.powered = true;
                    setChanged(pLevel, pPos, pState);
                }
            } else {
                try (Transaction transaction = Transaction.openOuter()) {
                    blockEntity.energyStorage.extract(energyCost, transaction);
                    transaction.commit();
                }
                if (blockEntity.tick == 200) {
                    pLevel.playSound(null, pPos, ModSounds.RADAR_SEARCH_IDLE, SoundSource.BLOCKS, 1.0F, 1.0F);
                }
    ...
    protected void saveAdditional(@NotNull CompoundTag pTag) {
        ...
        pTag.putLong("Energy", this.energyStorage.getAmount());
