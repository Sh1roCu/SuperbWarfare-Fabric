...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    private ItemModelBuilder simpleItem(Item item, String location) {
        ResourceLocation loc = BuiltInRegistries.ITEM.getKey(item);
        return withExistingParent(loc.getPath(), new ResourceLocation("item/generated"))
                .texture("layer0", Mod.loc("item/" + location + loc.getPath()));
    }

    private ItemModelBuilder simpleItem(Item item, String location, String renderType) {
        ResourceLocation loc = BuiltInRegistries.ITEM.getKey(item);
        return withExistingParent(loc.getPath(), new ResourceLocation("item/generated"))
                .texture("layer0", Mod.loc("item/" + location + loc.getPath())).renderType(renderType);
    }

    public void evenSimplerBlockItem(Block block) {
        this.withExistingParent(Mod.MODID + ":" + BuiltInRegistries.BLOCK.getKey(block).getPath(),
                modLoc("block/" + BuiltInRegistries.BLOCK.getKey(block).getPath()));
    }

    private ItemModelBuilder gunBlueprintItem(Item item) {
        return withExistingParent(BuiltInRegistries.ITEM.getKey(item).getPath(), new ResourceLocation("item/generated"))
                .texture("layer0", Mod.loc("item/gun_blueprint"));
    }

    private ItemModelBuilder cannonBlueprintItem(Item item) {
        return withExistingParent(BuiltInRegistries.ITEM.getKey(item).getPath(), new ResourceLocation("item/generated"))
                .texture("layer0", Mod.loc("item/cannon_blueprint"));
    }

    private ItemModelBuilder handheldItem(Item item) {
        return withExistingParent(BuiltInRegistries.ITEM.getKey(item).getPath(), new ResourceLocation("item/handheld"))
                .texture("layer0", Mod.loc("item/" + BuiltInRegistries.ITEM.getKey(item).getPath()));
    }

    private ItemModelBuilder gunIcon(Item item, String name) {
        return withExistingParent(BuiltInRegistries.ITEM.getKey(item).getPath() + "_icon", new ResourceLocation("item/generated"))
                .texture("layer0", Mod.loc("item/" + name + "_icon"));
    }

    private ItemModelBuilder gunBase(Item item, String name) {
        return getBuilder(BuiltInRegistries.ITEM.getKey(item).getPath() + "_base")
                .parent(new ModelFile.UncheckedModelFile(modLoc("displaysettings/" + name + ".item")))
                .texture("layer0", Mod.loc("item/" + name));
    }

    private ItemModelBuilder customSeparatedGunModel(Item item, String name) {
        ...
        return getBuilder(BuiltInRegistries.ITEM.getKey(item).getPath())
    ...
    public void gunItem(Item item) {
        this.gunItem(item, BuiltInRegistries.ITEM.getKey(item).getPath());
    }
