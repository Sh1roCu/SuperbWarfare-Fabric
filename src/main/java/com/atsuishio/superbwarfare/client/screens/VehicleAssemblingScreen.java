...
import cn.sh1rocu.superbwarfare.mixin.accessor.ScreenAccessor;
...
public class VehicleAssemblingScreen extends AbstractContainerScreen<VehicleAssemblingMenu> {
    ...
    public void initRecipes() {
        ...
        List<VehicleAssemblingRecipe> recipeList = recipeManager.getAllRecipesFor(ModRecipes.VEHICLE_ASSEMBLING_TYPE);
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        ((ScreenAccessor) this).sw$getRenderables().stream().filter(w -> w instanceof RecipeButton || w instanceof CategoryButton)
                .forEach(w -> {
    ...
    public void addAssembleButton(int posX, int posY) {
        this.addRenderableWidget(new AssembleButton(posX + 295, posY + 163, b -> {
            ...
            new AssembleVehicleMessage(this.currentRecipe.getId(), this.menu.containerId).send();
        }));
    }
