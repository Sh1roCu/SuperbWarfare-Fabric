...
public class KillMessageOverlay {
    ...
    public static void render(GuiGraphics guiGraphics, float partialTick) {
        ...
        switch (pos) {
            ...
            case RIGHT_TOP -> {
                posX = guiGraphics.guiWidth() - KillMessageConfig.KILL_MESSAGE_MARGIN_X.get();
                posY = KillMessageConfig.KILL_MESSAGE_MARGIN_Y.get();
            }
            case LEFT_BOTTOM -> {
                posX = KillMessageConfig.KILL_MESSAGE_MARGIN_X.get();
                posY = guiGraphics.guiHeight() - KillMessageConfig.KILL_MESSAGE_MARGIN_Y.get() - 10;
                left = true;
                bottom = true;
            }
            case RIGHT_BOTTOM -> {
    ...
    private static float renderKillMessages(LivingKillRecord record, GuiGraphics guiGraphics, float partialTick, int width, float baseTop, boolean left, boolean bottom) {
        ...
        if (!left) {
            int currentPosX = width - targetNameWidth - 10;
    ...
    public static String getEntityName(Entity entity) {
        ...
        if (entity instanceof LivingEntity living && living instanceof OwnableEntity ownableEntity && ownableEntity.getOwner() instanceof Player player) {
            if (DisplayConfig.DOG_TAG_NAME_VISIBLE.get()) {
                ...
                TrinketsApi.getTrinketComponent(player).flatMap(
                    ...
                    if (s.getB().hasCustomHoverName()) {
                        name[0] = s.getB().getHoverName().getString() + " + " + entityName;
                    }
                });
            } else {
                name[0] = player.getDisplayName().getString() + " + " + entityName;
            ...
            TrinketsApi.getTrinketComponent(player).flatMap(
                    c -> c.getEquipped(ModItems.DOG_TAG).stream().findFirst()).ifPresent(s -> {
                if (s.getB().hasCustomHoverName()) {
                    name[0] = s.getB().getHoverName().getString();
                }
            });
        }
        return name[0];
