...
public class ProjectileEntityModel extends GeoModel<ProjectileEntity> {
    ...
    public ResourceLocation getModelResource(ProjectileEntity entity) {
        ...
        if ((ClientEventHandler.zoom && !player.getMainHandItem().is(ModItems.MINIGUN))
                || player.getMainHandItem().is(ModItems.GLOCK_17)
                || player.getMainHandItem().is(ModItems.GLOCK_18)
                || player.getMainHandItem().is(ModItems.BOCEK)
